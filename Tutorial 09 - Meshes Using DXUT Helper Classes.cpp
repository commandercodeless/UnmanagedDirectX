...
//**********************************************************************//
float		 g_f_TigerRY            = XMConvertToRadians(45);  //45� default
float		 g_f_TigerRX			= XMConvertToRadians(45);  //45� default
                                      ...
                                      void* pUserContext )
{
    ...
    // Load the mesh.															//
	//**************************************************************************//
    V_RETURN( g_MeshTiger.Create(pd3dDevice, L"Media\\dog\\dog.sdkmesh", true ) );
                                  ...
                                  float fElapsedTime, void* pUserContext )
{
	...
	//**************************************************************************//
	XMVECTOR Eye = XMVectorSet( (g_f_TigerX- XMVectorGetX(vecArse)) * 10, (g_f_TigerY +1+ XMVectorGetY(vecArse)) * 10, (g_f_TigerZ - XMVectorGetZ(vecArse)) * 10, 0.0f );
	XMVECTOR At = XMVectorSet(g_f_TigerX * 10, g_f_TigerY * 10, g_f_TigerZ * 10, 0.0f);
	...
	//**************************************************************************//
	RenderMesh (pd3dImmediateContext, &g_MeshTiger, g_pPixelShader);
	...
	XMMATRIX mSpin = XMMatrixRotationZ(sin(timeGetTime() / 1000.0));

	if (sin(timeGetTime()/1000.0) > 0.75){
		PlaySound(L"Media\\Ride.wav", NULL, SND_ASYNC | SND_NOSTOP);
	}

	XMMATRIX matWingWorld = mSpin * matDogFix;
	XMMATRIX matWingWorldViewProjection;

	matWingWorld *= matTigerWorld;
	matWingWorldViewProjection = matWingWorld * matView * g_MatProjection;

	CBMatrices.matWorld = XMMatrixTranspose(matWingWorld);
	CBMatrices.matWorldViewProj = XMMatrixTranspose(matWingWorldViewProjection);

	pd3dImmediateContext->UpdateSubresource(g_pcbVSPerObject, 0, NULL, &CBMatrices, 0, 0);
	pd3dImmediateContext->VSSetConstantBuffers(0, 1, &g_pcbVSPerObject);

	RenderMesh(pd3dImmediateContext, &g_MeshWing, g_pSkyPixelShader);
	...
	XMVECTOR plane = XMVectorSet(1, 0, 0, 0);
	XMMATRIX matr = XMMatrixReflect(plane);

	XMMATRIX matWing1World = mSpin * matDogFix2 * matr;
	XMMATRIX matWingWorld1ViewProjection;

	matWing1World *= matTigerWorld;
	matWingWorld1ViewProjection = matWing1World * matView * g_MatProjection;

	CBMatrices.matWorld = XMMatrixTranspose(matWing1World);
	CBMatrices.matWorldViewProj = XMMatrixTranspose(matWingWorld1ViewProjection);
